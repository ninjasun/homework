...
(define (sw-corner size) (make-pattern size
                size
                (lambda (row col)
                  (if (>= row col)
                      #\*
...
(define (append-cols pattern1 pattern2)
		(define rowLen (min (pattern-numrows pattern1) (pattern-numrows pattern2)))
		(define colLen (+ (pattern-numcols pattern1) (pattern-numcols pattern2)))
		(make-pattern 
			rowLen
			colLen
			(lambda (row col) 
...
(define (append-rows pattern1 pattern2)
		(define rowLen (+ (pattern-numrows pattern1) (pattern-numrows pattern2)))
		(define colLen (min (pattern-numcols pattern1) (pattern-numcols pattern2)))
		(make-pattern 
			rowLen
			colLen
			(lambda (row col)
				(if (< row (pattern-numrows pattern1))
					((pattern-fn pattern1) row col)
					((pattern-fn pattern2) (- row (pattern-numrows pattern2)) col)
				)
			)
		)
)
...
(define (flip-cols pattern)
	(define rowLen (pattern-numrows pattern))
	(define colLen (pattern-numcols pattern))
	(make-pattern
         rowLen
         colLen
		(lambda (row col)
			(pattern-fn pattern) row (- (- colLen 1) col)
		)
  )
)
...
(define (flip-rows pattern)
	...
	(make-pattern
         ...
         colLen
		(lambda (row col)
			(pattern-fn pattern) (- (- rowLen 1) row) col
		)
  )
)
